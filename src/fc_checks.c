...
struct check_result* check_file_contexts_duplicate_entry(
		const struct check_data *data, const struct policy_node *node) {
	...
	if (node->flavor == NODE_FC_ENTRY) {
		...
		/***********************************************************************************
		 * By design, out should never be NULL because the fc_entry_map is pre-populated   *
		 * from the parse_fc_file method which is called before this function is called.   *
		 * By the time this function is called there will always be an entry in the        *
		 * fcs entry map for each file context entry. Unless somehow there was an error    *
		 * parsing an entry which the parse_fc_file method i believe should have caught    *
		 * return an selint parse error.                                                   *
		 * in any case I decided to add this check because during unit testing I forgot    *
		 * to first populate the fc_map prior to calling that function and that resulted   *
		 * to a segfault.                                                                  *
		 ***********************************************************************************/
		if (out && !is_the_registered_entry(out, node->lineno,
						data->filename)) {
